...
test ('Client login locator', async ({page}) => {
...
//}
await page.locator('.card-body').filter({hasText:itemtoAdd}).getByRole("button",{name:"Add To Cart"}).click()
...
//}
await page.locator("tr.ng-star-inserted").filter({hasText:orderid.replace(/\|/g, "").trim()}).getByRole("button",{name:"View"}).click();
await expect(page.getByText(orderid.replace(/\|/g, "").trim())).toBeVisible();
